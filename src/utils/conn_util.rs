...
pub fn with_conn<F, R>(variant:ConnVariant, action: F) -> Result<R, Error>
    ...
    match variant {
        ConnVariant::DeviceManager => 
            Ok(action(c.with_proxy("org.mconnect", "/org/mconnect/manager", Duration::from_millis(10)))),
        ConnVariant::Device(path) => Ok(action(c.with_proxy("org.mconnect", path, Duration::from_millis(10))))
    }
}